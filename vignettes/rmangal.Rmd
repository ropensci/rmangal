...
rmangal is a R client for MANGAL API. MANGAL is a collection of 172 datasets representing over [1300
ecological networks](http://poisotlab.biol.umontreal.ca/#/). In 2016, a first paper was published
with the first release of rmangal [@poisot_mangal_2016]. Since then, the database structure has been
improved, the API re-written and several ecological networks have been added. 
...
So far, `rmangal` package provides methods to get access to the datastore. All `httr::GET`
methods (to retrieve informations) don't require any authentification.

A bearer authentification strategy using ORCID credentials (as a third party services) has been
implemented on all `POST`, `DELETE`, `PUT` API operations. New functionnalities allowing networks
publication and using those API operations will be implemented in the next release (v2.2). 
